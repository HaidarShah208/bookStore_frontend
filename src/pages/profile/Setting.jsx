...
function Setting() {
  ...
  const handleSave = async () => {
    try {
      setLoading(true)
      await instance.put("/update", editedUser);
      setUser(editedUser);
      setIsEditing(false);
      setLoading(false)
      ...
      setLoading(false)
      toast.error("Error updating user:", error.response?.message);
      console.error("Error updating user:", error.response?.message);
    }
  };
    ...
    >
      ...
      <div className="w-full md:w-[60%] lg:w-[50%] space-y-6">
        ...
        <div className="space-y-4">
              <div>
                <h3 className="text-gray-300 text-xl pb-3 font-semibold">Name</h3>
                {isEditing ? (
                  <input
                    type="text"
                    name="username"
                  ...
                  <p className="text-white ps-2">{user.username}</p>
                )}
              </div>

              <div>
                <h3 className="text-gray-300 text-xl pb-3 font-semibold">Email</h3>
                {isEditing ? (
                  <input
                    type="email"
                    name="email"
                  ...
                  <p className="text-white ps-2">{user.email}</p>
                )}
              </div>

              <div>
                <h3 className="text-gray-300 text-xl pb-3 font-semibold">Address</h3>
                {isEditing ? (
                  <textarea
                    name="address"
                  ...
                  <p className="text-white ps-2">{user.address}</p>
                )}
              </div>
            </div>
            ...
            {isEditing ? (
              ...
              <button
                onClick={handleEdit}
                className="bg-zinc-600 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded"
              >
                Edit
              </button>
            )}
